 ...
 */
function getOrCreateRoom(config) {
    let room = gameRooms.get(config.tableId);
    if (!room) {
        const initialState = (0, game_state_1.createInitialTableState)(config.tableId, config.smallBlind, config.bigBlind);
...
io.on('connection', (socket) => {
    ...
    socket.on('create-table', (data) => {
        try {
            ...
            const config = Object.assign(Object.assign({}, data), { tableId, isQuickplay: false });
    ...
    socket.on('take-seat', (data) => {
        try {
            ...
            const player = (0, game_state_1.createPlayer)(playerId, spectator.name, seatPosition, buyIn);
